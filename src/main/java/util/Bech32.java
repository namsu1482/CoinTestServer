...
public class Bech32 {
    ...
    public static String getScriptPubKeyFromBech32Address(String bech32Address) {
        byte[] dataBytes = Bech32.bech32Decode(bech32Address).data;
        dataBytes = Utils.hexStringToByteArray(Utils.byteArrayToHex(dataBytes).substring(2));

        StringBuilder stringBuilder = new StringBuilder();
        for (int i = 0; i < dataBytes.length; i++) {
            stringBuilder.append(Utils.bit5ToBinaryString(dataBytes[i]));

        }
        byte[] scriptPubKey = new byte[20];
        String pubKeyBinaryString = stringBuilder.toString();
        for (int i = 0; i < scriptPubKey.length; i++) {
            int startIndex = i * 8;
            int endIndex = startIndex + 8;
            byte data = Utils.binaryStringToByte(pubKeyBinaryString.substring(startIndex, endIndex));
            scriptPubKey[i] = data;
        }
        return Utils.byteArrayToHex(scriptPubKey);
